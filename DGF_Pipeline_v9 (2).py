...
# # DGF Analytics Pipeline — v9 (DB-sourced admin + cleanup)
# 
# **v9:** B2B now loads from the **Zoho API** (incremental sync) instead of a local `Invoice (10).csv`. Output `B2B` is in the identical 173-column Invoice format, so every downstream cell (Section 6 onward) runs **unchanged**.
...
# ════════════════════════════════════════════════════════════════════════
# IMPORTS  (consolidated — all libraries used across the pipeline)
# ════════════════════════════════════════════════════════════════════════
...
# ## Loading Zoho through api

# In[2]:


# ════════════════════════════════════════════════════════════════════════
# ZOHO CREDENTIALS + token sanity check
# ════════════════════════════════════════════════════════════════════════
import requests
import pandas as pd

CLIENT_ID     = "1000.SH8C19MA1Y8B5EFMLDIITO3NHOS7IP"
CLIENT_SECRET = "18fef8bd17c7844f9ba3329934fe8cdb4f7a0fe9b3"
REFRESH_TOKEN = "1000.5bfe9f0da4afa9f1573fb7af36c43018.ccae5b792cf1827e40d8804f19c9f107"
ORG_ID        = "60068564826"
...
# ════════════════════════════════════════════════════════════════════════
# B2B FETCH FROM ZOHO API - WITH AUTH TOKEN FIX
# ════════════════════════════════════════════════════════════════════════
# ✅ FIXED: Generates fresh OAuth token from refresh token
...
# ════════════════════════════════════════════════════════════════════════
# ✅ FIX #1: Generate OAuth token from refresh token
# ════════════════════════════════════════════════════════════════════════
def get_oauth_token():
    """
    Generate fresh OAuth access token from refresh token.
    This fixes the 401 authentication error!
    """
...
# 28 clean columns we pull from the API
RAW_COLUMNS = [
...
def save_sync_state(last_time, count):
    """Save sync state ONLY after successful fetch."""
    with open(SYNC_STATE_FILE, "w") as f:
        json.dump({
            "last_sync_time": last_time,
            "last_invoice_count": count,
...
def load_master():
    ...
    if os.path.exists(MASTER_FILE):
        try:
            ...
            print(f"   ⚠️  Could not load master file")
            return None
    return None

def get_existing_invoice_ids(master_df):
    """Get set of invoice IDs already in master file."""
...
def get_invoice_ids(token, sync_date=None, date_start=FULL_LOAD_START):
    """
    ✅ Fetch ALL invoice IDs (we'll filter locally)
    """
    ...
    while True:
        ...
        try:
            ...
            if r.status_code == 401:
                print(f"   ❌ 401 Error: Authentication failed. Token may be expired.")
                print(f"      Try refreshing your refresh token.")
                break

            if r.status_code == 400:
                print(f"   ❌ 400 error on page {page}")
                break
...
def get_invoice_detail(invoice_id, token, retries=3):
    """Fetch full invoice details with retry."""
    headers = {"Authorization": f"Zoho-oauthtoken {token}"}
    ...
    for attempt in range(retries):
        try:
            r = requests.get(url, headers=headers,
                             params={"organization_id": ORG_ID}, timeout=15)
            r.raise_for_status()
            data = r.json()
...
# ════════════════════════════════════════════════════════════════════════
print("="*70)
print("🚀 B2B ZOHO FETCH - OPTIMIZED INCREMENTAL (SUPER FAST!)")
print("="*70)

# ✅ STEP 0: Generate OAuth token first!
print(f"\n[0] Generating OAuth token...")
token = get_oauth_token()

if not token:
    print(f"\n❌ FATAL: Could not generate OAuth token!")
    print(f"   Check your credentials (CLIENT_ID, CLIENT_SECRET, REFRESH_TOKEN)")
    exit(1)

# [STEP 1] Load existing master + sync state
print(f"\n[1] Loading existing data...")
master = load_master()
existing_ids = get_existing_invoice_ids(master)
sync_state = load_sync_state()

# [STEP 2] Fetch ALL invoice IDs from Zoho (fast, only IDs)
...
print(f"   ⏱️  Time saved: Not re-fetching {len(existing_ids):,} invoices!")

if len(new_ids) == 0:
    print(f"\n   ℹ️  No new invoices to fetch. Using existing master.")
    raw = master if master is not None else pd.DataFrame(columns=RAW_COLUMNS)
    B2B = to_invoice_format(raw)
    print(f"\n   📊 B2B ready (no changes): {len(B2B):,} rows")
else:
    # [STEP 4] Fetch ONLY new invoices
    print(f"\n[4] Fetching {len(new_ids):,} new invoices...")
    new_rows = []
    for i, inv_id in enumerate(new_ids, 1):
        if i % 50 == 0 or i == len(new_ids):
            pct = (i / len(new_ids)) * 100
            print(f"      ⏳ {i}/{len(new_ids)} ({pct:.0f}%)")
    ...
    # [STEP 5] Merge with existing + save
    ...
    # Save sync state ONLY after successful save
    save_sync_state(datetime.now().isoformat(), raw['invoice_id'].nunique())

    # Convert to Invoice format
    B2B = to_invoice_format(raw)

print(f"\n[6] ✅ B2B READY (173-col Invoice format):")
print(f"    {B2B.shape[0]:,} rows × {B2B.shape[1]} columns")
print(f"    💰 Revenue: ₹{pd.to_numeric(B2B['Item Total'], errors='coerce').sum():,.0f}")

print(f"\n{'='*70}")
print(f"✨ Next run will be EVEN FASTER - only new invoices fetched!")
print(f"{'='*70}\n")
...
# Quick sanity check that B2B has the columns Cell 29 expects
...
print(B2B[['Invoice Date','Invoice Number','Customer Name','Item Name','Quantity','Item Total']].head())


# ## LOADING DB CONNECITON THROUGH CEREDENTIALS

# In[5]:


# ── Database connection (moved from end of notebook for Section 15)
env_content = """DB_HOST=dgfdb.clsykici051f.ap-south-1.rds.amazonaws.com
DB_PORT=5432
DB_NAME=dgfdb
DB_USER=readonly_user
DB_PASSWORD=Gurugram@2026
"""
...
# ════════════════════════════════════════════════════════════════════════
# FINAL PERFECT DOD COGS PIPELINE - VERIFIED & WORKING
# ════════════════════════════════════════════════════════════════════════
# ✅ Matches admin panel exactly (B2B: 85,998.56 on 6/28)
# ✅ Full DOD data from Jan 2026 till today
# ✅ 3 sheets for complete visibility
# ════════════════════════════════════════════════════════════════════════
...
def push_to_sheet(df, sheet_id, tab_name):
    ...
    print(f"   🔄 Converting data to JSON-safe format...")
    ...
    print(f"   🔄 Clearing old data from {tab_name}...")
    ...
    print(f"   🔄 Writing {len(df_copy):,} rows to {tab_name}...")
    ...
    print(f"   ✅ {len(df_copy):,} rows pushed!")

def get_full_dod_query(start_date, end_date):
    """Get complete DOD COGS query - VERIFIED CORRECT."""
    return f"""
WITH order_totals AS (
    SELECT
        oi.order_id,
        SUM(oi.price * oi.item_quantity) AS order_gross
    ...
    INNER JOIN public.orders o 
        ...
        AND (
            o.payment_status = 1
            OR o.payment_mode IN ('COD','POD')
            OR o.is_draft_order = true
        )
...
sold_items AS (
    SELECT
        ...
        CASE 
            WHEN c.is_btob = true THEN 'B2B'
            ELSE 'B2C'
        END AS channel,

        CASE
            WHEN o.payment_status = 0 THEN 'Pending'
            WHEN o.payment_status = 1 THEN 'Success'
            ELSE 'Unknown'
        END AS payment_status,

        CASE
            WHEN o.order_status = 0 THEN 'Unfulfilled'
            WHEN o.order_status = 1 THEN 'Fulfilled'
            WHEN o.order_status = 2 THEN 'Cancelled'
            ELSE 'Unknown'
        ...
        SUM(
            (
                (oi.price * oi.item_quantity)
                -
                (
                    (oi.price * oi.item_quantity)
                    / NULLIF(ot.order_gross,0)
                    * COALESCE(o.discount_amount,0)
                )
                +
                COALESCE(oi.delivery_charge,0)
            )
...
pc_buying_cost AS (
    SELECT
        ...
        SUM(
            oi.item_quantity::numeric
            * COALESCE(recipe_cost.cost_per_unit, 0)
        ) AS total_buying_price
    ...
    LEFT JOIN LATERAL (
        SELECT
            SUM(
                recipe.recipe_qty
                * COALESCE(batch_price.avg_unit_price, fallback_price.avg_unit_price, 0)
            ) AS cost_per_unit
        FROM (
            SELECT DISTINCT ON (wfi.raw_id)
                wfi.raw_id,
                wfi.quantity::numeric AS recipe_qty
            FROM public.work_flow_output wfo
            JOIN public.work_flow_input wfi
                ON wfi.workflow_id = wfo.workflow_id
                AND wfi.status = true
            WHERE wfo.product_id = oi.item_id
              AND wfo.status = true
        ) recipe
        ...
        LEFT JOIN LATERAL (
            SELECT
                CASE
                    WHEN SUM(b.consumed_qty) > 0
                    THEN SUM(b.consumed_qty * gi.unit_price) / SUM(b.consumed_qty)
                    ELSE NULL
                END AS avg_unit_price
            FROM public.order_item_raw_batch_usage b
            JOIN public.grn_items gi
                ON gi.grn_item_id = b.grn_item_id
                AND gi.status = true
            WHERE b.order_item_id = oi.id
              AND b.raw_id = recipe.raw_id
              AND b.status = true
        ) batch_price ON true
        ...
        LEFT JOIN LATERAL (
            ...
            WHERE gi.rawitem_id = recipe.raw_id
              AND gi.status = true
              AND gi.unit_price > 0
...
SELECT
    ...
    ROUND(
        (
            si.total_items_sold::numeric /
            CASE
                WHEN LOWER(COALESCE(si.unit_name,'')) IN ('gm','gram','grams','g')
                THEN 1000
                ELSE 1
            END
        ) * si.product_qty,
    ...
    ROUND(
        COALESCE(pc.total_buying_price,0)::numeric,
        2
    ) AS total_buying_price,

    ROUND(
        (si.total_selling_price - COALESCE(pc.total_buying_price,0))::numeric,
        2
    ) AS total_margin,

    CASE
        WHEN si.total_selling_price > 0 THEN
            ROUND(
                (
                    (si.total_selling_price - COALESCE(pc.total_buying_price,0))
                    / si.total_selling_price * 100
                )::numeric,
                2
            )
        ELSE 0
...
LEFT JOIN pc_buying_cost pc
    ON pc.product_id = si.product_id
    AND pc.order_date = si.order_date
    AND pc.channel = si.channel
    AND pc.payment_status = si.payment_status
...
print("\n" + "="*70)
print("✅ FINAL PERFECT DOD COGS PIPELINE")
print("="*70)

start_date = '2026-01-01'
end_date = date.today().strftime('%Y-%m-%d')

print(f"\n📅 Fetching DOD data from {start_date} to {end_date}...")

try:
    # [1] Fetch all data
    print(f"\n[1] Fetching all data from database...")
    df_all = pd.read_sql(get_full_dod_query(start_date, end_date), engine)
    print(f"    ✅ {len(df_all):,} rows fetched")

    # [2] Push SHEET 1: Full data (all channels combined)
    print(f"\n[2] Pushing SHEET 1: Full DOD Data...")
    push_to_sheet(df_all, SHEETS_ID, 'COGS_Daily_Full')

    # [3] Create and push SHEET 2: Daily summary by channel
    print(f"\n[3] Creating SHEET 2: Daily Summary by Channel...")
    ...
    daily_summary = daily_summary.sort_values('order_date', ascending=False)
    push_to_sheet(daily_summary, SHEETS_ID, 'COGS_Daily_Summary')

    # [4] Create and push SHEET 3: B2B only (for admin panel comparison)
    print(f"\n[4] Creating SHEET 3: B2B Channel Only (matches admin panel)...")
    ...
    print(f"\n📊 OVERALL SUMMARY (Jan 2026 - Today):")
    print(f"   Total rows: {len(df_all):,}")
    print(f"   Total revenue: ₹{df_all['total_selling_price'].sum():,.2f}")
    print(f"   Total COGS: ₹{df_all['total_buying_price'].sum():,.2f}")
    print(f"   Total margin: ₹{df_all['total_margin'].sum():,.2f}")

    print(f"\n📊 BREAKDOWN BY CHANNEL:")
    by_channel = df_all.groupby('channel').agg({
        'total_selling_price': 'sum',
        'total_buying_price': 'sum',
        'total_margin': 'sum'
    }).round(2)
    ...
    print(f"\n✅ VERIFICATION (6/28/2026):")
    b2b_6_28 = df_b2b[df_b2b['order_date'] == '2026-06-28']['total_buying_price'].sum()
    print(f"   B2B COGS: ₹{b2b_6_28:,.2f}")
    print(f"   Admin shows: ₹85,998.56")
    print(f"   Match: {'✅ YES!' if abs(b2b_6_28 - 85998.56) < 0.01 else '❌ NO'}")

    print(f"\n📋 3 SHEETS CREATED:")
    print(f"   1️⃣  COGS_Daily_Full - All products, all channels, daily")
    print(f"   2️⃣  COGS_Daily_Summary - Daily totals by channel (clean view)")
    print(f"   3️⃣  COGS_B2B_Only - B2B channel only (for admin comparison)")

    print(f"\n💡 HOW TO USE:")
    print(f"   • Use COGS_Daily_Full for detailed analysis")
    print(f"   • Use COGS_Daily_Summary for quick daily view")
    print(f"   • Use COGS_B2B_Only to verify against admin panel")
...
# ## Loading Customer report from db

# In[7]:


# ════════════════════════════════════════════════════════════════════════
# LOAD ADMIN DATA from database (replaces Excel download)
# ════════════════════════════════════════════════════════════════════════

# Date range: from Dec 1, 2025 till today. Change START_DATE if needed.
...
SELECT
    TO_CHAR(
        o.created_date AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata',
    ...
    TO_CHAR(
        oi.exp_delivery_datetime AT TIME ZONE 'UTC' AT TIME ZONE 'Asia/Kolkata',
        'YYYY-MM-DD'
    ) AS delivery_date,

    (
        CASE
            WHEN pws.from_time IS NOT NULL 
             AND pws.to_time IS NOT NULL
            THEN pws.from_time::TEXT || ' - ' || pws.to_time::TEXT
            ELSE pws.slot_name
        END
    ...
    c.mobile_no AS customer_contact,
    p.title AS product_title,
    o.note AS notes,

    (
        CASE
            WHEN o.is_draft_order = true THEN 'draftorder'
            ELSE o.channel
        END
    ) AS sales_channel,
    ...
    oi.item_quantity AS net_items_sold,
    oi.is_express_delivery,
    store.store_name,
    ...
    ROUND(
        ...
        2
    )::NUMERIC(12,2) AS net_sales,

    ROUND(
        COALESCE(oi.gst_amount, 0),
        2
    )::NUMERIC(12,2) AS taxes,

    ROUND(
        COALESCE(oi.delivery_charge, 0),
        2
    )::NUMERIC(12,2) AS shipping_charges,
...
INNER JOIN public."darkStore_packagingStore" store 
    ON store.id = oi.store_id
...
LEFT JOIN public.pincode_wise_slot pws 
    ON pws.id = oi.slot_id
...
try:
    admin = pd.read_sql(text(query), engine)
    print(f"✅ Loaded admin data from DB: {len(admin):,} rows | {len(admin.columns)} columns")
    print(f"   Date range: {admin['order_created_date'].min()} → {admin['order_created_date'].max()}")
    print(f"   Columns: {', '.join(admin.columns.tolist()[:8])}...")
except Exception as e:
    print(f"❌ Failed to load admin from DB: {e}")
    print("   Falling back to Excel if available...")
    try:
        admin = pd.read_excel(r"D:\downloads\Customer_Order_Report (65).xlsx")
        print(f"✅ Loaded from Excel fallback: {len(admin):,} rows")
    except:
        print("❌ Excel fallback also failed. Define admin manually or check DB connection.")
        admin = None


# ## DIAGNOSTIC — verify DB status columns

# In[8]:


# ════════════════════════════════════════════════════════════════════════
# DIAGNOSTIC — confirm DB status columns are text (not numeric codes)
# normalize_status (Section 11) expects text like 'paid'/'fulfilled'.
# If these come back as numbers, send me the values and I'll add the
# code→label translation so revenue classification stays correct.
# ════════════════════════════════════════════════════════════════════════
if admin is not None:
    for col in ['payment_status', 'fulfillment_status_code']:
        if col in admin.columns:
            print(f"{col}  (dtype={admin[col].dtype}):")
            print(admin[col].value_counts(dropna=False).head(15).to_string())
            print()
...
def clean_phone(phone):
    """
    Universal phone cleaner. Returns:
      - 10-digit Indian number (preferred) — e.g. '9811213001'
      - International number with '+' preserved — e.g. '+447911123456'
      - Original cleaned string for garbage (8-digit etc.) — preserved for ops review
      - None for truly empty/null

    Handles: apostrophe prefix ('), backtick prefix (`), +91 / 91 / 0 prefix,
             scientific notation, trailing .0, spaces, dashes, brackets, dots.
    """
    if pd.isna(phone):
        return None
    ...
    # Strip apostrophe prefix (Excel/Sheets text-force) — loop handles ''+91...
    while s.startswith("'"):
        s = s[1:].strip()

    # Strip backtick prefix (rare but seen)
    while s.startswith("`"):
        s = s[1:].strip()

    # Trailing .0 only (NOT anywhere — old code had bug stripping mid-string .0)
    if s.endswith('.0'):
        s = s[:-2]

    # Scientific notation rescue
    ...
    # Detect international BEFORE stripping '+'
    ...
    # Indian path: strip all formatting chars including '+'
    s = re.sub(r'[\s\-\(\)\.\+]', '', s)

    if not s or s.lower() == 'nan':
        return None

    # Normalize Indian prefixes to 10 digits
...
def clean_email(email):
    """Lowercased, stripped email. Returns None for invalid."""
    if pd.isna(email):
        return None
...
# ── Shopify Orders
orders_1 = read_csv_from_zip(r"D:\downloads\orders_export_1 (2).zip")
orders_2 = read_csv_from_zip(r"D:\downloads\orders_export_2 (2).zip")
orders_3 = read_csv_from_zip(r"D:\downloads\orders_export_3 (2).zip")
orders_shopify = pd.concat([orders_1, orders_2, orders_3], ignore_index=True)

# ── Shopify Customers
customers_shopify = read_csv_from_zip(r"D:\downloads\customers_export (2).zip")

# ── Admin Panel
# admin = pd.read_excel(r"D:\downloads\Customer_Order_Report (65).xlsx")
# ── B2B Invoices now come from the Zoho API cell above (no local file)
zip_shopify = pd.read_csv(r"D:\downloads\zip_shopify.csv")
Indian_zip_codes = pd.read_csv(r"D:\downloads\Indian zip codes_modified.zip.csv")

customer_id_shopify = pd.read_csv(r"D:\downloads\Orders by customer email (1).csv")
...
print(f"   Zip_shopify:               {zip_shopify.shape}")
print(f"   Indian_zip_codes:               {Indian_zip_codes.shape}")


# ## SECTION 2 — CLEAN SHOPIFY ORDERS
...
# ## SECTION 3 — CLEAN SHOPIFY CUSTOMERS
...
# ## SECTION 5 — CLEAN ADMIN DATA (Blinkit + B2B Detection)
...
# ## CREATING THE SKU COLUMN IN ADMIN
...
# ## SECTION 6 — CLEAN B2B DATA (Zoho)
# 
# - `Item Total` (line-level) → `Lineitem price`
# - `SubTotal` / `Total` (invoice-level) → only on FIRST line of each invoice
# - `Invoice Status` → `Fulfillment Status` (Void=Voided, rest=Completed handled in Section 11)
# 
...
# ## SECTION 7 — COMBINE INTO MASTER_ORDERS
...
# ## SECTION 8 — Reclassify B2B_DraftOrder from 1P → B2B
...
# ## SECTION 9 — CUSTOMER KEY (Coalesce + Propagate)
...
# ## SECTION 10 — ⚡ NET REVENUE (LINE-ITEM LEVEL)
# 
# **This is the key fix vs v3** — earlier `Net_Revenue` was just a copy of order-level `Total`.
# 
# Now:
# - **1P:** `Lineitem_Revenue = qty × price` → Proportional discount → `Net_Revenue = Lineitem_Revenue − Lineitem_Discount`
# - **B2B:** `Net_Revenue = Lineitem price` (already line-level from Section 6)
# 
...
# ## SECTION 11 — ORDER STATUS MAPPING
# 
# - **1P:** Fuzzy mapping of Financial + Fulfillment status
# - **B2B (Zoho):** From `Fulfillment Status` (Invoice Status)
#   - `Void` → `Voided`
#   - `Open` / `Closed` / `Overdue` / anything else → `Completed`
# 
...
# ## SECTION 11B — Adjust Net_Revenue for Refunded / Voided orders
# 
# Business rules now that we have a final `order_status`:
# 
# | order_status | Net_Revenue treatment |
# |---|---|
# | **Completed** | Keep as computed (positive) |
# | **Refunded**  | Negate — money returned to customer (e.g., +₹199 → −₹199) |
# | **Voided**    | Zero — money never moved |
# | **Cancelled / Pending** | Keep as-is (these are filtered out at dashboard level) |
# 
# We also preserve the pre-adjustment value as `Gross_Revenue` for reference.
...
# ## SECTION 11C — Order Time-of-Day columns
# 
# Add two columns based on the `Created at` timestamp:
# 
# - **`Order Hour`** — integer hour 0–23 (for hour-by-hour charts in Power BI)
# - **`Order Time Period`** — `"Before 3:30 PM"` or `"After 3:30 PM"` (the express-eligibility split)
# 
# Used to answer:
# - % of orders placed BEFORE 3:30 PM (current cutoff rule)
# - % of orders placed AFTER 3:30 PM (the express opportunity)
# - Peak-hour browsing patterns
...
# ## SECTION 12 — CUSTOMER SEGMENTATION (NCA / RC2 / RC3 / LC)
...
# ## SECTION 13 — CUSTOMER TRANSITIONS (Order Level)
...
# ## Fetch `zip_admin` from DB
# 
# Pulls a fresh `order_id`, `order_name`, `address`, `pincode` mapping for every
# admin order. This refreshes automatically each time the pipeline runs.
# 
# > **IMPORTANT:** The SQL below includes `o.order_name` so we can match admin
# > orders by their `#DN...` style name. If your `orders` table uses a different
# > column name for the display name (e.g. `name`, `order_number`), update the
# > SQL accordingly.
...
# ## SECTION 15 — ⚡ ADDRESS CLEANUP (Lookup-Based) — REWRITTEN
# 
# **Three data sources** feed the final address columns:
# 
# | Source | Used for | Key match | Output |
# |---|---|---|---|
# | `zip_shopify` | Shopify sub-channels | `Order name` ↔ `Name` | postal_code |
# | `zip_admin` (from DB) | Admin + B2B_DraftOrder sub-channels | `order_name` ↔ `Name` | pincode |
# | `Indian_zip_codes` (Kaggle) | Everyone | pincode lookup | city, area, district, state |
# 
# **Five output columns** added to `master_orders`:
# - `zip` — clean 6-digit pincode
# - `city` — most common city for that pincode
# - `area` — first post office area for that pincode
# - `state` — most common state for that pincode
# - `district` — most common district for that pincode
# 
# Old `PIN_PREFIX_MAP` and `CITY_FALLBACK` logic is replaced — the Kaggle dataset
# covers all Indian pincodes with proper district/area data.
...
# ## SECTION 17 — DROP REDUNDANT ADDRESS COLUMNS
...
# ## SECTION 18 — LIGHT CLEANING FOR POWER BI
...
# ## SECTION 19 — CONSERVATIVE DEDUPLICATION (100% identical rows only)
...
# ## Adding the first order date & LAST ORDER DATE in the master orders 
...
# ## Checking if the cancelled at is not blank to make the order status cancelled
...
PARQUET_PATH = r"D:\downloads\master_orders.parquet"
master_orders.to_parquet(PARQUET_PATH, index=False, engine="pyarrow")
print(f"✅ Saved: {PARQUET_PATH} ({round(os.path.getsize(PARQUET_PATH)/1024/1024, 2)} MB)")


# ## FETCHING THE DELIVERY TIME SLOT FOR SHoPIFY DATA 
...
# ## SECTION 20 — PUSH TO GOOGLE SHEETS
...
# # ==================== CONFIG ====================
# LOOKBACK_DAYS = 90
# INCLUDE_B2B   = False  # True kar de agar Zomato Hyperpure etc. bhi chahiye
# OUTPUT_PATH   = r'D:\downloads\frequency_segment_90d.xlsx'

# # ==================== STEP 1: Prep ====================
# df = master_orders.copy()
# df['Created at'] = pd.to_datetime(df['Created at'], errors='coerce')

# # Agar timezone aware hai toh strip kar de (clean comparison ke liye)
# if df['Created at'].dt.tz is not None:
#     df['Created at'] = df['Created at'].dt.tz_localize(None)

# df = df.dropna(subset=['Created at', 'customer_key'])

# # ==================== STEP 2: Filter ====================
# cutoff = pd.Timestamp.now() - pd.Timedelta(days=LOOKBACK_DAYS)

# mask = (df['order_status'] == 'Completed') & (df['Created at'] >= cutoff)
# if not INCLUDE_B2B:
#     mask &= (df['channel'] == '1P')

# recent = df[mask].copy()
# print(f"Filtered rows: {len(recent):,} line items in last {LOOKBACK_DAYS} days")

# # ==================== STEP 3: Aggregate per customer ====================
# # Note: 'Name' is order-level identifier (#1001 type). nunique = distinct orders.
# customer_agg = recent.groupby('customer_key').agg(
...
#     state             = ('state',             'first'),
#     pincode           = ('zip',               'first'),
#     order_count_90d   = ('Name',              'nunique'),    # <-- counts distinct orders
#     first_order_90d   = ('Created at',        'min'),
#     last_order_90d    = ('Created at',        'max'),
#     total_spend_90d   = ('Net_Revenue',       'sum'),
#     total_qty_90d     = ('Lineitem quantity', 'sum'),
#     first_order_ever  = ('first_order_date',  'first'),
#     total_lifetime    = ('total_lifetime_orders','max'),
# ).reset_index()

# # Derived metrics
# customer_agg['days_since_last_order'] = (pd.Timestamp.now() - customer_agg['last_order_90d']).dt.days
# customer_agg['avg_order_value']       = (customer_agg['total_spend_90d'] / customer_agg['order_count_90d']).round(0)

# # Avg gap (sirf un customers ke liye jinke 2+ orders hain)
# denom = (customer_agg['order_count_90d'] - 1).replace(0, np.nan)
# customer_agg['avg_gap_days'] = ((customer_agg['last_order_90d'] - customer_agg['first_order_90d']).dt.days / denom).round(1)

# # ==================== STEP 4: Bucket ====================
# def bucket(n):
#     if n >= 9:   return '1_weekly'
#     if n >= 5:   return '2_bi_weekly'
#     if n >= 2:   return '3_monthly'
#     return '4_one_time'

# customer_agg['frequency_segment'] = customer_agg['order_count_90d'].apply(bucket)

# # ==================== STEP 5: Summary ====================
# summary = customer_agg.groupby('frequency_segment').agg(
#     customer_count   = ('customer_key',    'count'),
#     avg_orders       = ('order_count_90d', 'mean'),
#     total_orders     = ('order_count_90d', 'sum'),
#     total_revenue    = ('total_spend_90d', 'sum'),
#     avg_rev_per_cust = ('total_spend_90d', 'mean'),
# ).round({'avg_orders': 1, 'total_revenue': 0, 'avg_rev_per_cust': 0})

# summary['pct_of_active'] = (summary['customer_count'] / summary['customer_count'].sum() * 100).round(1)

# # ==================== STEP 6: Print + Export ====================
# print("\n" + "="*70)
# print(f"FREQUENCY SEGMENTATION — LAST {LOOKBACK_DAYS} DAYS")
# print(f"Channel: {'1P only' if not INCLUDE_B2B else '1P + B2B'}")
# print("="*70)
# print(summary)
# print()
# print(f"TOTAL ACTIVE CUSTOMERS : {len(customer_agg):,}")
# print(f"TOTAL ORDERS           : {customer_agg['order_count_90d'].sum():,}")
# print(f"TOTAL REVENUE          : ₹{customer_agg['total_spend_90d'].sum():,.0f}")

# # Excel export: 1 summary sheet + 1 sheet per segment + 1 all-in-one
# with pd.ExcelWriter(OUTPUT_PATH, engine='openpyxl') as writer:
#     summary.to_excel(writer, sheet_name='Summary')

#     customer_agg.sort_values(['frequency_segment', 'order_count_90d'],
#                               ascending=[True, False]) \
#                 .to_excel(writer, sheet_name='All_Active_Customers', index=False)

#     for seg, sheet in [('1_weekly','Weekly'),('2_bi_weekly','Bi_Weekly'),
#                        ('3_monthly','Monthly'),('4_one_time','One_Time')]:
#         customer_agg[customer_agg['frequency_segment']==seg] \
#             .sort_values('order_count_90d', ascending=False) \
#             .to_excel(writer, sheet_name=sheet, index=False)

# print(f"\n✓ Saved to: {OUTPUT_PATH}")


# ## Pasting Data in Saksham Sheet
...
# DATE CONVERSION
# ==========================
df_push["Created at"] = pd.to_datetime(
    df_push["Created at"],
    errors="coerce"
)
...
# Existing Order IDs from Column B
...
if not new_orders.empty:
    ...
    # Check filled rows in column A
    col_a = sheet.col_values(1)

    # If only header exists, start at row 2
    if len(col_a) <= 1:
        start_row = 2
    else:
    ...
    print("ℹ No new 1P orders found to push.")
